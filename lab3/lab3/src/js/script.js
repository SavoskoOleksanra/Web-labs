(function () {
    ...
    var checkName = function () {
        ...
        if ((/[^a-zA-Z0-9]/.test(name1.value))) {
            var error = generateError('Invalid symbols');
    ...
    var checkNum = function () {
        ...
        if (parseInt(num.value) != num.value) {
            var error = generateError('Invalid num');
    ...
    var checkNumR = function () {
        ...
        for (var i = 0; i < arr.length - 2; i++) {
            if ((arr[i] == 's') && (arr[i + 1] == 'q') && (arr[i + 2] == 'r')) {
                var error = generateError('Invalid numR');
...
function getExistingUsers() {
    ...
    .then(data => {
        ...
        if (parseInt(realData.count) === 0) {
          document.getElementById('message').innerHTML = "There are no users!";
        } else {
            document.getElementById('message').innerHTML = "Users";
            ...
            res.forEach(elem => {
                ...
                newLi.innerHTML = `
                    ...
                    Num = ${elem.num}<br>
                    NumR = ${elem.numr}<br>
                    Password = ${elem.password}<br>
                    PasswordConfirmation = ${elem.passwordConfirmation}
                `;
